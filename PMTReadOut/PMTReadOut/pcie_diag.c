...
*/
  ...
  printf(" 3) write-to-file DMA test \n");
  scanf("%d",&newcmd);
  switch(newcmd) {
...
/** start the receiver **/
      ...
      WDC_WriteAddr32(hDev, dwAddrSpace, dwOffset, u32Data);
       ...
       //      buffp_send = pbuf_send;
       //      buffp_rec = pbuf_rec;
       //comment out; instead, do rand() generation outside the loop, and fill array(s) ahead of time
      ...
      */
      ...
      //      iseq==1 by definition for this test
      /*
      if(iseq == 1) {
...
/***    check to see if DMA is done or not **/
	...
	break;
      ...
      //Uncomment the following to check data in == data out
/*      if(idone == 1) { */
/* 	buffp_rec = pbuf_rec; */
/* 	buffp_send = pbuf_send; */
/* 	for (i=0; i<nwrite; i++) { */
...
/*       } */
  ...
  case 3:
    printf(" enter number of 32bits word send (< 20000)\n");
    scanf("%d",&nwrite);
    printf("enter number loops \n");
    scanf("%d",&iloop);
    
    printf(" Using DMA read after DMA write \n");

    const int narr = nwrite;
    UINT32 my_write_array[narr],my_read_array[narr];
...
/* clear DMA register after the abort */
    ...
    /*   if(k == 7 ) printf("\n"); */
    /* } */
    /* printf("\nReceiver data (words) before loop-back test\n"); */
    /* for (i=0; i<nwrite; i++) { */
    /*   k = i%8; */
    /*   if( k ==0) printf(" %3d", i); */
    /*   u32Data = *buffp_rec++; */
    ...
    printf("\n\nWriting to file: -----------> testDMA.bin\n");

    FILE * pFile4;
    pFile4 = fopen("testDMA.bin","w");
    if (pFile4==NULL){
    ...
    /** begin timer **/
    struct timeval starttest4, endtest4;
    gettimeofday(&starttest4,NULL);
    long mytime4, seconds4, useconds4;
    seconds4 = starttest4.tv_sec;
    useconds4 = starttest4.tv_usec;
...
/* synch DMA i/O cache **/
	...
	buffp_send = pbuf_send;
      ...
      //This is write to disk method 1

      //           write receiver data to an array
      for (i=0; i<nwrite; i++){
	my_read_array[i]=*buffp_send++;
	my_write_array[i]=*buffp_rec++;
      	//	printf(" %llX\t %llX\n",read_array[i],write_array[i]);
      }

      //write to file
      //      printf("size of write array %i\n",sizeof(write_array));
      fwrite(&my_write_array, sizeof(my_write_array), 1, pFile4);

    }//end loop

    /** end timer **/
    gettimeofday(&endtest4,NULL);
    seconds4 = endtest4.tv_sec;
    useconds4 = endtest4.tv_usec;
      
    printf("\nEnd time of test:   %ld sec %ld usec\n",seconds4,useconds4);

    seconds4 = endtest4.tv_sec - starttest4.tv_sec;
    useconds4 = endtest4.tv_usec - starttest4.tv_usec;
      
    mytime4 = seconds4*1000000. + useconds4*1.; //elapsed time in usec

    printf("\nElapsed time:\t\t\t %ld usec\n\n", mytime4);

    fclose(pFile4);

    printf("\nLast array of words was:\n");
    for (i=0; i<nwrite; i++){
      //      printf(" %llX\t %llX\n",my_read_array[i],my_write_array[i]);
    }
