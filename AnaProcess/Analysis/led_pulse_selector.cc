...
led_pulse_selector::led_pulse_selector() : _ref_time() { 
  ...
  _ref_ch=PMT::INVALID_CH;
  _frame_size=PMT::EMPTY_WORD;
  _time_cut=PMT::EMPTY_WORD;
  clear_event();
}
...
bool led_pulse_selector::select_pulse(const pulse_info* data){
  ...
  store= false;
  PMT::word_t time_stamp = data->frame_id() * _frame_size + data->timeslice();
