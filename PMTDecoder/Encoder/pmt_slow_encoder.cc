...
void pmt_slow_encoder::encode_event_header(const event_waveform* data) {
  ...
  _words[_index]  = format(PMT::EVENT_HEADER,(data->nwords())>>12);
  _words[_index] += (format(PMT::EVENT_HEADER,(data->nwords()) & 0xfff))<<16;
  _index++;
  
  // (3) Third 32-bit word include event number
  _words[_index]  = format(PMT::EVENT_HEADER,(data->event_id())>>12);
  _words[_index] += (format(PMT::EVENT_HEADER,(data->event_id()) & 0xfff))<<16;
  _index++;

  // (4) Forth 32-bit word include frame number
  _words[_index]  = format(PMT::EVENT_HEADER,(data->event_frame_id())>>12);
  _words[_index] += (format(PMT::EVENT_HEADER,(data->event_frame_id()) & 0xfff))<<16;
  _index++;
