...
bool algo_slow_readout_decoder::process_word(PMT::word_t word) {
  ...
  case READ_CHANNEL:
    ...
    if(word_class==FORMAT::EVENT_HEADER || 
       word_class==FORMAT::EVENT_FIRST_HEADER) {
...
bool algo_slow_readout_decoder::process_header(const PMT::word_t word) {
  ...
  //
  FORMAT::PMT_WORD word_class=get_word_class(word);
  ...
  case FORMAT::EVENT_HEADER:
    ...
    if(get_word_class(word>>16)!=FORMAT::EVENT_HEADER) {
...
bool algo_slow_readout_decoder::process_ch_word(const PMT::word_t word,
  ...
  // extract information
  FORMAT::PMT_WORD last_word_class=get_word_class(last_word);
  FORMAT::PMT_WORD word_class=get_word_class(word);
  ...
  if(word_class==FORMAT::UNDEFINED_WORD){
    sprintf(_buf,"Found an undefined word: %x ",word);
    if(last_word_class!=FORMAT::CHANNEL_LAST_WORD && 
       last_word_class!=FORMAT::FEM_LAST_WORD &&
       last_word_class!=FORMAT::EVENT_LAST_WORD)
      Message::send(MSG::WARNING,__FUNCTION__,_buf);
    else{ 
      if(_verbosity[MSG::INFO])
  ...
  if(!status){
    ...
    if(word_class==FORMAT::EVENT_LAST_WORD){
	...
	status=true;
      ...
      init_event_info();
    }else if(word_class!=FORMAT::FEM_LAST_WORD){
      _nwords-=1;
      _checksum-=word;
    }
...
bool algo_slow_readout_decoder::decode_ch_word(const PMT::word_t word, 
				       const FORMAT::PMT_WORD last_word_class)  {
  ...
  case FORMAT::FIRST_WORD:
    ...
    if(last_word_class!=FORMAT::EVENT_HEADER){
      Message::send(MSG::ERROR,__FUNCTION__,
  ...
  case FORMAT::CHANNEL_HEADER:
    ...
    if(last_word_class!=FORMAT::FIRST_WORD && last_word_class!=FORMAT::CHANNEL_LAST_WORD) {
      Message::send(MSG::ERROR,__FUNCTION__,
  ...
  case FORMAT::CHANNEL_LAST_WORD:
	...
	if(word_class!=FORMAT::CHANNEL_LAST_WORD)
      ...
      }else if(last_word_class==FORMAT::CHANNEL_HEADER   ) {
	...
	_channel_header_count++;
      }else if(last_word_class==FORMAT::CHANNEL_WORD     ) {
