...
public:
  ...
  void set_data_to_read(PMT::DATA_TYPE type,bool read=true)
  ...
  /// Setter for I/O file name. ... deprecated
  //void set_filename(std::string name) {_in_fname=name;};
  ...
  //std::string filename() const {return input_filename();};

  /// Getter for I/O file name.
  //std::string input_filename() const {return _in_fname;};
...
private:
  ...
  /// 
  void create_data_ptr(PMT::DATA_TYPE type);

  void delete_data_ptr(PMT::DATA_TYPE type);
  ...
  bool _read_data_array[PMT::DATA_TYPE_MAX];

  /// Boolean to record what data to be written out from a file
  bool _write_data_array[PMT::DATA_TYPE_MAX];

  /// Data pointer array
  data_base *_ptr_data_array[PMT::DATA_TYPE_MAX];
  ...
  TChain *_in_ch[PMT::DATA_TYPE_MAX];
  TTree  *_out_ch[PMT::DATA_TYPE_MAX];

  //
  // Event-wise data storage class instances
  //

  /// waveform collection
  //event_waveform* _event_wf;

  /// trigger data
  //trig_info* _trig_info;
