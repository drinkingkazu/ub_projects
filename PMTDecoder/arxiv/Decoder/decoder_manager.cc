...
bool decoder_manager::decode() {
  ...
  }else if(_storage->get_event_waveform().size()){
    Message::send(MSG::WARNING,
		  __FUNCTION__,
		  "Last event not stored by algorithm. Missing end-of-event word??");
    if(_decoder->check_event_quality()){
      sprintf(_buf,"Last event checksum agreed. Saving on file... event id: %d",_storage->get_event_waveform().event_id());
      Message::send(MSG::WARNING,__FUNCTION__,_buf);
      _storage->next_event();
    }else{
      sprintf(_buf,"Skip saving the last event: %d ",_storage->get_event_waveform().event_id());
      Message::send(MSG::WARNING,__FUNCTION__,_buf);
      status=false;
    }    
  }
