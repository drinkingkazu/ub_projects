...
bool beamgate_debugger::analyze(const storage_manager* data){
  ...
  for(std::vector<pmt_waveform>::const_iterator wf_iter(data->get_event_waveform().begin());
      wf_iter!=data->get_event_waveform().end();
      ++wf_iter){
