...
bool pmtbaseline::analyze(const storage_manager* storage) {
  //New event
  clear_event();
  ...
  const event_waveform ewform = storage->get_event_waveform();
  ...
  ((storage_manager*)storage)->get_pulse_collection_writeable()->set_sum_charge(event_charge   );  
  ((storage_manager*)storage)->get_pulse_collection_writeable()->set_sum_peak  (event_amplitude);  
  ((storage_manager*)storage)->get_pulse_collection_writeable()->set_npulse    (npulse         );
