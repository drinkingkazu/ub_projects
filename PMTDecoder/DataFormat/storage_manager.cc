  ...
  : decoder_base(), _treename("pmt_data")
{
  ...
  for(size_t i=0; i<PMT::DATA_TYPE_MAX; ++i) {
  ...
  _read_data_array[PMT::EVENT_WF]=true;
  _write_data_array[PMT::EVENT_WF]=true;
...
bool storage_manager::prepare_tree(){
  ...
  std::vector<uint64_t> nevents_array(PMT::DATA_TYPE_MAX,0);
  ...
  for(size_t i=0; i<PMT::DATA_TYPE_MAX; ++i){
    ...
    if(_mode!=WRITE && _read_data_array[i]) {
      ...
      _in_ch[i]=new TChain(PMT::DATA_TREE_NAME[(PMT::DATA_TYPE)i].c_str(),
			   Form("%s Tree",PMT::DATA_TREE_NAME[(PMT::DATA_TYPE)i].c_str()));
	...
	create_data_ptr((PMT::DATA_TYPE)i);
    ...
    if(_mode!=READ && _write_data_array[i] ) {
      ...
      _out_ch[i]=new TTree(PMT::DATA_TREE_NAME[(PMT::DATA_TYPE)i].c_str(),
			   Form("%s Tree",PMT::DATA_TREE_NAME[(PMT::DATA_TYPE)i].c_str()));
  ...
  for(size_t i=0; i<PMT::DATA_TYPE_MAX; ++i) {
    ...
    if(nevents_array[i] && _nevents!=nevents_array[i]) {
      sprintf(_buf,"Different number of entries found on tree: %s",PMT::DATA_TREE_NAME[(PMT::DATA_TYPE)i].c_str());
      Message::send(MSG::ERROR,__FUNCTION__,_buf);
      status=false;
    }
  ...
  /*  
  if(_mode==READ){
    if(!(_fin->Get(_treename.c_str()))){
      sprintf(_buf,"Tree \"%s\" not found in the file: %s",_treename.c_str(),_in_fname.c_str());
      Message::send(MSG::ERROR,__FUNCTION__,_buf);
      return false;
    }
    _event_wf=new event_waveform;
    _trig_info=new trig_info;

    _in_tree=(TTree*)(_fin->Get(_treename.c_str()));
    if(_read_data_array[FORMAT::EVENT_WF])
      _in_tree->SetBranchAddress(_event_wf->GetName(),&_event_wf);
    if(_read_data_array[FORMAT::TRIG_INFO])
      _in_tree->SetBranchAddress(_trig_info->GetName(),&_trig_info);
    _nevents=_in_tree->GetEntries();
    _nevents_read=0;
    _index=0;
  }else if(_mode==WRITE){
    _event_wf=new event_waveform;
    _trig_info=new trig_info;
    _out_tree=new TTree(_treename.c_str(),"pmt data tree");
    if(_write_data_array[FORMAT::EVENT_WF])
      _out_tree->Branch(_event_wf->GetName(),_event_wf->GetName(),&_event_wf);
    if(_write_data_array[FORMAT::TRIG_INFO])
      _out_tree->Branch(_trig_info->GetName(),_trig_info->GetName(),&_trig_info);
    _nevents_written=0;
    _index=0;
  }else if(_mode==BOTH){
    if(!(_fin->Get(_treename.c_str()))){
...
void storage_manager::create_data_ptr(PMT::DATA_TYPE type){
...
void storage_manager::delete_data_ptr(PMT::DATA_TYPE type){
...
bool storage_manager::close(){
  ...
  if(status) {
    ...
    for(size_t i=0; i<PMT::DATA_TYPE_MAX; ++i) {
      ...
      if(_ptr_data_array[i]) delete_data_ptr((PMT::DATA_TYPE)i);
