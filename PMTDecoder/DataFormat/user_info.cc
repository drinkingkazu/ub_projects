...
void user_info::dump_contents(){
  ...
  Message::send(MSG::NORMAL,__FUNCTION__," Start contents dump...");
  std::string msg("");
  ...
  Message::send(MSG::NORMAL,__FUNCTION__," End of dump...");
}
...
double user_info::get_double(std::string key) const {  
  ...
  if(item==_d_map.end()){
    Message::send(MSG::ERROR,__FUNCTION__,Form("Key \"%s\" does not exist!", key.c_str()));
    return 0;
  }
  return (*item).second;
}
